...
import {
    ...
    FileCode, Package, Wrench, Palette, Layout, Cpu, HardDrive,
...
// Allowed extensions for file filtering
const allowedExtensions = ['js', 'jsx', 'ts', 'tsx', 'html', 'css', 'json', 'md', 'txt', 'py', 'java', 'cpp', 'c', 'php', 'rb', 'go', 'rs'];
...
const StructuredResultsView = ({ results, onApply, onBack, viewTitle, projectFiles, applyingId, setApplyingId, readOnly = false }) => {
...
const App = () => {
    ...
    */
    ...
    // The functions `addNotification` and `dismissNotification` are defined here
    // to be used by other parts of the App component.
    ...
    useEffect(() => {
        localStorage.setItem('ai-code-assistant-projects', JSON.stringify(projects.map(p => ({
            ...p,
            dirHandle: undefined, // Cannot serialize dirHandle
            files: p.files.map(f => ({ ...f, handle: undefined })) // Cannot serialize handles
        }))));
    }, [projects]);
    ...
    */
    ...
    const executeAITask = useCallback(async (taskType, options = {}) => {
        if (!projectFiles.length) {
            addNotification('Please open a project folder first.', 'warning');
            return;
        }
    ...
    // Recursive directory reading using File System Access API
const readDirectory = async (dirHandle, path = '') => {
    const files = [];
    let completed = 0;
    const entries = [];
    const excludedDirs = ['node_modules', '.git', 'dist', 'build']; // Add directories to exclude here
    ...
    for (const handle of entries) {
        ...
        if (handle.kind === 'directory' && !excludedDirs.includes(handle.name)) {
            files.push(...await readDirectory(handle, relPath));
            ...
            if (allowedExtensions.includes(ext)) {
                ...
                files.push({
                    ...
                    size: file.size,
                    type: file.type,
                    handle // Store the FileSystemFileHandle for writing
                });
            }
        }
        completed++;
...
const handleDeleteProject = useCallback((projectId) => {
    ...
    if (selectedProject?.id === projectId) {
        ...
        addNotification('Project deleted and deselected.', 'info');
    } else {
...
const handleOpenFolder = useCallback(async () => {
    if (!('showDirectoryPicker' in window)) {
        addNotification('File System Access API not supported in this browser. Use Chrome or Edge.', 'error');
        return;
    }
    ...
    try {
        ...
        setLoading(true);
        setCurrentTask('Reading project folder...');
        setProgress(0);

        const uploadedFiles = await readDirectory(dirHandle);

        const newProjectName = dirHandle.name;
        ...
        addNotification(`Opened project: "${newProjectName}" with ${uploadedFiles.length} file(s)`, 'success');
...
const handleSaveToDisk = useCallback(async () => {
  if (!selectedProject || !projectFiles.some(f => f.handle)) {
    addNotification('No local folder opened or no file handles available.', 'warning');
    return;
  }

  setLoading(true);
  setCurrentTask('Saving changes to disk...');
  setProgress(0);
  ...
  try {
    ...
    for (const file of projectFiles) {
      if (file.handle) {
        ...
        let currentHandle = selectedProject.dirHandle;
        for (const part of parts) {
          currentHandle = await currentHandle.getDirectoryHandle(part, { create: true });
        }
        file.handle = await currentHandle.getFileHandle(fileName, { create: true });
        const writable = await file.handle.createWritable();
        await writable.write(file.content);
        await writable.close();
      }
    ...
    addNotification('All changes saved to disk successfully.', 'success');
  } catch (error) {
...
}, [selectedProject, projectFiles, addNotification]);

const applyStructuredResult = useCallback(async (result) => {
  const existingFileIndex = projectFiles.findIndex(file => file.path === result.filePath);
  
  if (!result.filePath || result.filePath.includes('..') || !result.refactoredContent) { addNotification(`Invalid file path or content for ${result.filePath}`, 'error'); return; }
  ...
  if (existingFileIndex !== -1) {
    ...
    const newFile = {
      ...
      name: result.filePath.split('/').pop(), // Extract filename from path
      path: result.filePath,
      content: result.refactoredContent,
      size: new Blob([result.refactoredContent]).size, // Approximate size
      type: 'text/plain', // Default type, adjust based on extension if needed
      handle: null // Handle will be null for new files until saved
    };
  ...
  setSelectedFile(prev => 
    prev?.path === result.filePath 
      ? { ...prev, content: result.refactoredContent } 
      : (existingFileIndex === -1 ? newFiles[newFiles.length - 1] : prev)
  );
...
const fileTree = useMemo(() => {
    ...
    projectFiles.forEach(file => {
        if (!file?.path) return; // Guard against invalid paths
...
const Sidebar = () => (
    <div className="w-64 bg-slate-800 text-white p-4 h-full overflow-y-auto">
        ...
        {projects?.map(project => (
            <div key={project.id} className="mb-2">
                <button
                    ...
                    className="w-full text-left bg-slate-700 p-2 rounded hover:bg-slate-600"
                >
                    {project.name}
                </button>
                <button
                    onClick={() => handleDeleteProject(project.id)}
                    className="w-full bg-red-500 text-white p-2 rounded mt-1 hover:bg-red-600"
                >
                    Delete
                </button>
            </div>
            ...
            <header className="bg-slate-900/95 backdrop-blur-sm border-b border-slate-700/50 sticky top-0 z-40">
                <div className="flex items-center justify-between px-6 py-4">
                    ...
                    <div className="flex items-center space-x-4">
                        ...
                        {selectedProject?.files?.some(f => f.handle) && (
                            <button
                                onClick={handleSaveToDisk}
                                className="px-4 py-2 bg-gradient-to-r from-green-600 to-emerald-600 text-white rounded-lg hover:from-green-700 hover:to-emerald-700 transition-all duration-200 flex items-center space-x-2"
                            >
                        ...
                        <div className="p-6 h-full flex flex-col overflow-hidden">
                            ...
                            {selectedFile ? (
                                ...
                                <div className="bg-slate-800/30 rounded-xl border border-slate-700/50 p-12 text-center flex flex-col justify-center items-center flex-1">
                                    ...
                                    <p className="text-slate-500">Select a file from the sidebar or open a new project folder to begin.</p>
                                </div>
                            )}
                        </div>
                    )}
