...
import { QuestWithDefinition } from '../services/firestoreService'; // Adjust path if needed
...
const QuestMenu: React.FC<QuestMenuProps> = ({ quests, onQuestSelect }) => {
  ...
  const handleSelect = (quest: QuestWithDefinition) => {
    ...
    // We pass it an object with the planet's name, which the animator will use as a target.
    onQuestSelect({ planetName: quest.definition.name });
