    ...
    return (
        <group {...props} dispose={null} ref={planet} onClick={() => onPlanetClick(name)}>
            ...
            <mesh
                ...
                scale={0.02}
            >
                <meshLambertMaterial map={terrainBottom} emissive={isSelected ? new THREE.Color(0x4488ff) : new THREE.Color(0x000000)} emissiveIntensity={isSelected ? 0.3 : 0} />
            </mesh>
            <mesh
                ...
                scale={0.02}
            >
                <meshLambertMaterial map={terrainTop} emissive={isSelected ? new THREE.Color(0x4488ff) : new THREE.Color(0x000000)} emissiveIntensity={isSelected ? 0.3 : 0} />
            </mesh>
            {isSelected && (
                <mesh scale={0.022}> {/* Slightly larger scale for the rim */}
            ...
            {isSelected && (
                <mesh scale={2.02}> {/* Slightly larger scale for the rim */}
                    <sphereGeometry args={[1, 32, 32]} />
                    <meshBasicMaterial
                        color={0x88ccff}
                        transparent
